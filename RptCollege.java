...
class RptCollegeApp implements ActionListener
{
	JFrame f1;
	JPanel p1,p2;
	GridLayout gl51;
	...
	JButton btnClose;
	String colHead[]={"SL_No","College_Name","Principal_Name","Period","Locality","City","State","PhoneNo","Email_ID","WebSite"};//It is are Table Col_Name of Current Table
	Object objData[][];
	JTable tblData;
	JScrollPane jspData;//Not Working
	Connection conn;
	...
	public RptCollegeApp()
	{
		f1=new JFrame("College Information");
		p1=new JPanel();
		p2=new JPanel();
		gl51=new GridLayout(7,1);
		flw=new FlowLayout();
		
		lHead=new JLabel("College Information",JLabel.CENTER);
	...
	public void doConnect()
	{
		...
		try
		{
			...
			rs.last();//
			tot=rs.getRow();
			objData=new Object[tot][10];
			rs.beforeFirst();//
			
			while(rs.next())
			{
				objData[rw][0]=slno;
				objData[rw][1]=rs.getString("cname");
				objData[rw][2]=rs.getString("pname");
				objData[rw][3]=rs.getString("period");
				objData[rw][4]=rs.getString("locality");
				objData[rw][5]=rs.getString("city");
				objData[rw][6]=rs.getString("state");
				objData[rw][7]=rs.getString("phone");
				objData[rw][8]=rs.getString("email");
				objData[rw][9]=rs.getString("website");
				slno++;
				rw++;
			}
		}
...
class RptCollege
{
	...
	{
		RptCollegeApp rpt1=new RptCollegeApp();
	}
}